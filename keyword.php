...
*/
  ...
  echo '<a href=\'special.php?task='.$task.'&type='.$type.'&special_flag=1\'  value="specialurl" class="easyui-linkbutton">特殊URL</a>';
  echo '</form>';
  echo '</div>';
  ...
  if($form_source != ''){
	if(is_array($form_source)){
		...
		$sql_query = $sql_query.' and `comefrom` = "'.$form_source.'"';
		$sql_count = $sql_count.' and `comefrom` = "'.$form_source.'"';
	}
  }
  ...
  $data_count = 0;
	  ...
	  while($tKeyword = mysql_fetch_array($resKeyword)){
		...
		echo '<tr>
					...
					<td class="table-grid"><a href="#" class="easyui-linkbutton" onclick=\'showChart("'.$tKeyword['url'].'","'.$tKeyword['comefrom'].'","'.$tKeyword['task'].'","'.$tKeyword['type'].'","'.$tKeyword['keyword'].'","'.(date('Ymd')-7).'","'.date('Ymd').'")\'>'.substr($tKeyword['url'],0,40).'</a></td>
	...
	if(ceil($form_number/$form_page) <= 1){
		...
		echo '<td><a href=\'keyword.php?task='.$task.'&type='.$type.'&form_number=0 &keyword='.$form_keyword.'&source='.$form_source.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	...
	if(ceil($form_number/$form_page) <= 1){
		...
		echo '<td><a href=\'keyword.php?task='.$task.'&type='.$type.'&form_number='.((ceil(($form_number-$form_page)/$form_page)-1)*$form_page).'&keyword='.$form_keyword.'&source='.$form_source.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	echo '  <span class="l-btn-left"><span class="l-btn-text">
	...
	if(ceil($form_number/$form_page) >= ceil($data_count/$form_page)){
		...
		echo '<td><a href=\'keyword.php?task='.$task.'&type='.$type.'&form_number='.$form_number.'&keyword='.$form_keyword.'&source='.$form_source.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	echo '  <span class="l-btn-left"><span class="l-btn-text">
	...
	if(ceil($form_number/$form_page) >= ceil($data_count/$form_page)){
		...
		echo '<td><a href=\'keyword.php?task='.$task.'&type='.$type.'&form_number='.((ceil($data_count/$form_page)-1)*$form_page).'&keyword='.$form_keyword.'&source='.$form_source.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	echo '  <span class="l-btn-left"><span class="l-btn-text">
