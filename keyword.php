...
<?php
  ...
  $data_count = 0;
	  ...
	  while($tKeyword = mysql_fetch_array($resKeyword)){
		...
		if($tKeyword['flag_whitelist'] == 1){
			echo '<td class="table-grid" style="text-align:center">已加入</td>';
		}else{
			echo '			<td class="table-grid" style="text-align:center"><a href="#" class="easyui-linkbutton" onclick=\'whitelistadd("'.$tKeyword['url'].'","'.$tKeyword['comefrom'].'","'.$tKeyword['task'].'","'.$tKeyword['type'].'","'.$tKeyword['keyword'].'","'.$tKeyword['time'].'","'.$tKeyword['number'].'","'.$tKeyword['title'].'","keyword.php")\'>加入</a></td>';
		}
		echo '</tr>';
