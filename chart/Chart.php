<?php
	...
	{	
		$arrData_baidu[$i_baidu][1]=substr($t_baidu['time'],4);
	...
	{	
		$arrData_soso[$i_soso][1]=substr($t_soso['time'],4);
	...
	{	
		$arrData_google[$i_baidu][1]=substr($t_google['time'],4);
...
<html>
	...
	while($startdate_baidu <= $enddate){
		foreach ($arrData_baidu as $arSubData){
			$flag_baidu += 1;
			if($arSubData[1] == substr($startdate_baidu,4)){
				if($flag_baidu == 1){
					echo $arSubData[2];
					break;
				}else{
	...
	while($startdate_soso <= $enddate){
		...
		foreach ($arrData_soso as $arSubData){
			if($arSubData[1] == substr($startdate_soso,4)){
				if($flag_soso == 1){
					echo $arSubData[2];
					break;
				}else{
