...
function showChart_special(data_url, comefrom, task, type, keyword, startdate, enddate, sourcefile)
{
	...
	window.open("chart/Chart.php?url="+data_url+"&task="+task+" &type="+type+"&startdate="+startdate+" &enddate="+enddate+"\" &sourcefile="+sourcefile,'url_window','height=500,width=1100,top=150,left=150,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
...
*/
  ...
  while($tKeyword = mysql_fetch_array($special_url_sql)){
	...
	echo '
			...
			<td class="table-grid"><a href="#" class="easyui-linkbutton" onclick=\'showChart_special("'.$change_url.'","'.$tKeyword['comefrom'].'","'.$tKeyword['task'].'","'.$tKeyword['type'].'","'.$tKeyword['keyword'].'","'.$chart_date_start.'","'.$chart_date_end.'","specialurl")\'>'.substr($tKeyword['url'],0,40).'</a></td>
			...
			<td class="table-grid"><a href="#" class="easyui-linkbutton" onclick=\'whitelistadd("'.$change_url.'","'.$tKeyword['comefrom'].'","'.$tKeyword['task'].'","'.$tKeyword['type'].'","'.$tKeyword['keyword'].'","'.$tKeyword['time'].'","'.$tKeyword['number'].'","'.$tKeyword['title'].'","special.php")\'>加入</a></td>
		</tr>';
  }
  ...
  if(ceil($form_number/$form_page) <= 1){
	...
	echo '<td><a href=\'special.php?task='.$task.'&type='.$type.'&form_number=0 &source='.$comefrom.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
  }
	...
	if(ceil($form_number/$form_page) <= 1){
		...
		echo '<td><a href=\'special.php?task='.$task.'&type='.$type.'&form_number='.((ceil(($form_number-$form_page)/$form_page)-1)*$form_page).'&source='.$comefrom.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	echo '  <span class="l-btn-left"><span class="l-btn-text">
	...
	if(ceil($form_number/$form_page) >= ceil($data_count/$form_page)){
		...
		echo '<td><a href=\'special.php?task='.$task.'&type='.$type.'&form_number='.$form_number.'&source='.$comefrom.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	echo '  <span class="l-btn-left"><span class="l-btn-text">
	...
	if(ceil($form_number/$form_page) >= ceil($data_count/$form_page)){
		...
		echo '<td><a href=\'special.php?task='.$task.'&type='.$type.'&form_number='.((ceil($data_count/$form_page)-1)*$form_page).'&source='.$comefrom.'&start_date='.$form_start_date.'&end_date='.$form_end_date.'\' class="l-btn l-btn-plain">';
	}
	echo '  <span class="l-btn-left"><span class="l-btn-text">
