...
cur_del_special.execute('DELETE FROM `appear` WHERE 1')
...
cur_select_webpage_count_sixday = conn.cursor()
...
#print 'select distinct `task` from `timerecord` where `time` >= "%s" and `time` <= "%s"'%(startday,sixthday)
...
#    print 'select distinct `type` from `timerecord` where `task`="%s" and `time` >= "%s" and `time` <= "%s"'%(row_select_timerecord_task[0],startday,sixthday)
...
#        print 'select distinct `keyword` from `timerecord` where `task`="%s" and `type`="%s" and `time` >= "%s" and `time` <= "%s"'%(row_select_timerecord_task[0],row_select_timerecord_type[0],startday,sixthday)
...
#                print 'select count(*) from `timerecord` where `task`="%s" and `type`="%s" and `keyword`="%s" and `comefrom`="%s" and `time` >= "%s" and `time`<= "%s"'%(row_select_timerecord_task[0],row_select_timerecord_type[0],row_select_timerecord_keyword[0],source,startday,sixthday)
                rows_select_timerecord_count = cur_select_timerecord_count.fetchall()

                download_number = rows_select_timerecord_count[0][0]
#		print download_number//01010101
...
#                print 'select distinct `url` from `webpage` where `task`="%s" and `type`="%s" and `keyword`="%s" and `comefrom`="%s" and `time` >= "%s" and `time`<= "%s" and `flag_whitelist` != 1'%(row_select_timerecord_task[0],row_select_timerecord_type[0],row_select_timerecord_keyword[0],source,startday,today)
		    ...
		    cur_select_webpage_count_sixday.execute('select count(*) from `webpage` where `task`="%s" and `type`="%s" and `keyword`="%s" and `comefrom`="%s" and `url`="%s" and `time` >= "%s" and `time`<= "%s"'%(row_select_timerecord_task[0],row_select_timerecord_type[0], row_select_timerecord_keyword[0],source,row_select_webpage_url[0],startday,sixthday))
#                    print 'select count(*) from `webpage` where `task`="%s" and `type`="%s" and `keyword`="%s" and `comefrom`="%s" and `url`="%s" and `time` >= "%s" and `time`<= "%s"'%(row_select_timerecord_task[0],row_select_timerecord_type[0], row_select_timerecord_keyword[0],source,row_select_webpage_url[0],startday,sixthday)
                    rows_select_webpage_count = cur_select_webpage_count.fetchall()
                    url_count = rows_select_webpage_count[0][0]   #31231112
                    rows_select_webpage_count_sixday = cur_select_webpage_count_sixday.fetchall()
                    url_count_sixday = rows_select_webpage_count_sixday[0][0]
		    #print rows_select_webpage_count[0]
		    #if rows_select_webpage_count[0][0] ==0:
		#	print url_count_sixday
                    if ((download_number>0) and (url_count_sixday == 0) and (url_count > 0)):#有搜索记录且前6天count为0，前7天count大于0
